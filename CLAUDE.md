...
- `install_linkcatty.cmd` — `FILE_LIST[n]`, `TOTAL` and the `for /l` bound (the bar counts files + FFmpeg)
...
- **Launcher look (all four scripts + the two uninstallers):** same design as the app (`- a Maiz's one -`, bold title + dim version, rule, `✔ ⚠ ✖ ›` status lines, one in-place bar). `.cmd` files must stay ASCII, so they use `+ ! x >`, `#`/`.` bars and ANSI colors (enabled only on Windows 10+); the `.sh` files use the app's real glyphs. Each script carries its own small copy of the helpers (`ui_*`); keep them in sync by hand. In `.cmd` files pass text to the helpers through the `MSG` / `DET` variables, never as arguments (`!`, `%` and `&` in paths).
